...
#   OFFICE_NAME / OFFICE_ADDRESS 邀约到所面谈话术里报的所名与地址
#   DOUYIN_CLIENT_KEY / DOUYIN_CLIENT_SECRET / DOUYIN_CALLBACK_TOKEN
#                               抖音私信凭据（见 docs/douyin.md）
...
RESPONDER_DOUYIN_CLIENT_KEY=${DOUYIN_CLIENT_KEY:-}
RESPONDER_DOUYIN_CLIENT_SECRET=${DOUYIN_CLIENT_SECRET:-}
RESPONDER_DOUYIN_CALLBACK_TOKEN=${DOUYIN_CALLBACK_TOKEN:-}
RESPONDER_DOUYIN_DEFAULT_NOTIFY_USERID=${DOUYIN_DEFAULT_NOTIFY_USERID:-}
RESPONDER_OFFICE_NAME=${OFFICE_NAME:-上海松沪律师事务所}
RESPONDER_OFFICE_ADDRESS=${OFFICE_ADDRESS:-上海市松江区九峰路88号平高广场11楼}
...
EOF
  ...
  # 邀约到所面谈的话术要报全名和地址，改所址只需重跑本脚本并传 OFFICE_ADDRESS
  # 抖音私信：过审后把 client_key/secret/回调 Token 传进来重跑本脚本即可
  _ensure_env RESPONDER_DOUYIN_CLIENT_KEY "${DOUYIN_CLIENT_KEY:-}"
  _ensure_env RESPONDER_DOUYIN_CLIENT_SECRET "${DOUYIN_CLIENT_SECRET:-}"
  _ensure_env RESPONDER_DOUYIN_CALLBACK_TOKEN "${DOUYIN_CALLBACK_TOKEN:-}"
  _ensure_env RESPONDER_DOUYIN_DEFAULT_NOTIFY_USERID "${DOUYIN_DEFAULT_NOTIFY_USERID:-}"
  _ensure_env RESPONDER_OFFICE_NAME "${OFFICE_NAME:-上海松沪律师事务所}"
  _ensure_env RESPONDER_OFFICE_ADDRESS "${OFFICE_ADDRESS:-上海市松江区九峰路88号平高广场11楼}"
  ...
  for k in WECOM_BOT_TOKEN WECOM_BOT_AES_KEY BOT_DEFAULT_NOTIFY_USERID \
           ...
           PUBLIC_BASE_URL OFFICE_NAME OFFICE_ADDRESS \
           DOUYIN_CLIENT_KEY DOUYIN_CLIENT_SECRET DOUYIN_CALLBACK_TOKEN \
           DOUYIN_DEFAULT_NOTIFY_USERID; do
    v="$(eval echo "\${$k:-}")"
