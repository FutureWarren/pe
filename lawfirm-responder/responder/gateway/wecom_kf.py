...
ERR_HINTS: dict[int, str] = {
    ...
    # ---- 发送类错误码（2026-08-25 补）。
    # 这四个原本一个都没覆盖，于是发送失败在控制台里只是一句「发送失败」，
    # 而它们各自要做的事完全不同——一个要改流程，一个只能等客户再开口，
    # 一个是我们自己发多了。**分不清就修不了。**
    95018: (
        "这通会话已经交给人工接待（或在排队/已结束），企业微信不允许应用再往里发消息。"
        "官方规定 kf/send_msg 只在会话状态 0（新接入待处理）和 1（由智能助手接待）"
        "可用，且**状态 3 无法再转回 1**（只能转给别的接待人或结束会话）。"
        "所以「转人工之后 AI 继续陪聊」这条路在企微上走不通——"
        "要么别那么早转，要么接受转完就由真人负责。"
    ),
    95031: (
        "这位客户 48 小时内没有主动发过消息，企业微信不允许我们先开口。"
        "只能等他再发一句，窗口才会重新打开。"
    ),
    95002: (
        "超出 48 小时会话窗口：客户最后一次发消息已经过去 48 小时以上，"
        "这条发不出去。等他再开口窗口就重置。"
    ),
    95001: (
        "超出条数限额：客户每发一次消息，我们最多回 5 条。"
        "要么把话说得更聚合，要么调小分条数（`split_max_parts`）。"
    ),
    95003: (
        "主体未验证：企业没做认证、也没绑视频号时，能咨询的微信用户数有上限。"
        "去管理后台完成企业认证，或绑定视频号。"
    ),
    95014: "这位接待人还没激活企业微信（没登录过），先让他登录一次。",
    95019: (
        "转接失败：目标接待人当前不在「正在接待」状态。"
        "让他在企微「微信客服」里把接待开关打开。"
    ),
}

# `msg_send_fail` 回调的失败原因。
# **接口返回成功不代表送达**——企微是异步告知发送失败的，
# 只看 send_msg 的返回值会让库里标着「已发送」而客户一个字没收到。
# 这正是本项目反复强调的那类失败：日志正常、控制台正常、客户那头是空的。
SEND_FAIL_HINTS: dict[int, str] = {
    4: "超出 48 小时会话窗口，等客户再开口才能发。",
    5: "会话已经关闭了。",
    6: "超出条数限额（客户每发一次，最多回 5 条）。",
    8: "企业主体未验证，可咨询用户数已达上限。",
    10: "用户拒收了这个客服账号的消息。",
    11: (
        "**企业没有任何成员登录过企业微信 App。** 实战里这一条最常见，"
        "而症状具有欺骗性：接口一路返回成功，客户却什么也收不到。"
        "让任意一位同事在手机上登录一次企业微信即可。"
    ),
    12: "消息类型被组件禁发。",
    13: "触发了安全限制。",
}
...
def send_fail_hint(fail_type: int | None) -> str:
    """把 `msg_send_fail` 的 fail_type 翻成一句能照着做的中文。"""
    try:
        return SEND_FAIL_HINTS.get(int(fail_type), "") if fail_type is not None else ""
