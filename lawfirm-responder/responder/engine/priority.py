...
# 转过去 90 秒没人接手，AI 会自动接着陪（`handoff_grace_seconds`，
# 且不清转接状态，律师随时能接手）。转多了的代价只是企微里多几个待接窗口，
...
# 故意不在清单里：`fee`（只问收费）、`depth`（聊得久）、`deadline`（有时限）、
# `amount`（金额大）。这些说明**案子值钱**，不说明**客户想找人**——
# 转过去客户会愣一下（他只是问了句话），而 AI 本可以接着把案情摸清楚。
# 它们照常加分，照常影响律师的跟进顺序。
