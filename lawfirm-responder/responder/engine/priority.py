...
WANTS_HUMAN: list[tuple[str, str]] = [
    ...
    # 上面每一条都是**意愿型**信号：客户自己在伸手要人。这类一律立刻转，
    # 不设任何前置条件——让一个已经开口要律师的人先答完四道题才叫人，
    # 是本末倒置（应转尽转，2026-08-12 律所方拍板）。
    #
    # 下面这条是**信息型**：客户只是把事情说了出来，没要人。它同样要转
    # （不转就会流失掉那些不好意思开口的客户），但**要等 AI 把案情问清楚**。
    # 律所方 2026-08-13 的原话：「我们不能在客户都没有描述清楚案情的情况下
    # 就转接给人工啊，那人工还是得再问一轮。」
    # 第一稿把它绑在 `rules.has_substance`（说了点什么就转），结果客户随口
    # 一句「公司拖欠工资」就转过去，律师看到的还是那一句——观感是
    # 「有 AI 和没 AI 完全没区别」，准确。
    # 现在绑的是 `screening.ready`（四件事够门槛，或问到上限），
    # 由 service 在一对一进线窗口注入。
    ("screened", "客户把案情说清楚了（筛查达标）"),
]
